...
public partial class ActualPlayer : Node
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (Input.IsActionJustPressed("Number2")) { //LeftArm
            throwingLimb = LimbTypes.LeftArm; 
            if (starterLimbs[throwingLimb].amIsolated) selectedLimb.Deselect();
            selectedLimb = starterLimbs[throwingLimb].OnSelect(); 
            }
        ...
        if (Input.IsActionJustPressed("Number4")) { //RightArm
            throwingLimb = LimbTypes.RightArm; 
            if (starterLimbs[throwingLimb].amIsolated) selectedLimb.Deselect();
            selectedLimb = starterLimbs[throwingLimb].OnSelect(); 
            }
        if (Input.IsActionJustPressed("Number5")) { //LeftLeg
            throwingLimb = LimbTypes.LeftLeg; 
            if (starterLimbs[throwingLimb].amIsolated) selectedLimb.Deselect();
            selectedLimb = starterLimbs[throwingLimb].OnSelect(); 
            }
        if (Input.IsActionJustPressed("Number6")) { //RightLeg
            throwingLimb = LimbTypes.RightLeg; 
            if (starterLimbs[throwingLimb].amIsolated) selectedLimb.Deselect();
            selectedLimb = starterLimbs[throwingLimb].OnSelect(); 
            }
    ...
    {
        Vector3 mousePosition = mainCamera.ProjectPosition(GetViewport().GetMousePosition(), Math.Abs(mainCamera.Position.Z - ((Node3D)starterLimbs[LimbTypes.Torso]).Position.Z));
        Vector3 direction = ((Node3D)starterLimbs[LimbTypes.Torso]).Position.DirectionTo(new Vector3(mousePosition.X, mousePosition.Y, 0));
